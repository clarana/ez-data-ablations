...
export SOURCE=$5
# valid sources; s2orc-recombined, m2d2_wiki, or m2d2_wiki-toplevel
# s2orc_partition_idx-9-27.json, m2d2_wiki_partition_idx-3-19.json, or m2d2_wiki-top_partition_idx-3-19.json

wandb login --relogin $WANDB_KEY

# data (changes for other dataset classes)
export EXPT_BASE=seed40bil-branch-decon-$MODEL_SZ
...
export CHECKPOINT_FOLDER=/results/branch-xppt-${EXPT_BASE}/${SOURCE}/${PARTITION}
...
# MICRO_BATCH_SIZE needs to be smaller for 1b model
[[ $MODEL_SZ = 1_1b ]] && export MICRO_BATCH_SIZE=8 || export MICRO_BATCH_SIZE=16
...
python xp-pt-as-olmo/convert_checkpoint.py --checkpoint $checkpt --config xp-pt-as-olmo/configs/customgpt-${MODEL_SZ}.yaml --output_dir /net/nfs.cirrascale/allennlp/claran/catwalk-models/branched-train-${PARTITION//[, ]/}-decon-$MODEL_SZ
cp /net/nfs.cirrascale/allennlp/claran/catwalk-models/seed-decon-5epoch-${MODEL_SZ}/*.json /net/nfs.cirrascale/allennlp/claran/catwalk-models/branched-train-${PARTITION//[, ]/}-decon-$MODEL_SZ
