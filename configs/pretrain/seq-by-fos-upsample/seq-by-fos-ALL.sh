...
export EXPT_BASE=seed40bil-seq-combo-upsamp-decon-$MODEL_SZ

function join_by { local IFS="$1"; shift; echo "$*"; }
export SOURCES=("s2orc-recombined" "m2d2_wiki-toplevel")
export EXPT=${EXPT_BASE}-$(join_by "-" "${SOURCES[@]}")
...
export CHECKPOINT_FOLDER=/data/claran/xppt-${EXPT_BASE}/$(join_by "-" "${SOURCES[@]}")/${GROUP}
...
cp "$CHECKPOINT_FOLDER/$EXPT-metrics.json" /data/claran/metrics.json
...
python xp-pt-as-olmo/convert_checkpoint.py --checkpoint $checkpt --config xp-pt-as-olmo/configs/customgpt-${MODEL_SZ}.yaml --output_dir /net/nfs.cirrascale/allennlp/claran/catwalk-models/seq-combo-decon-$MODEL_SZ/seq-$(join_by "-" "${SOURCES[@]}")-ALL-upsamp
cp /net/nfs.cirrascale/allennlp/claran/catwalk-models/seed-decon-5epoch-${MODEL_SZ}/*.json /net/nfs.cirrascale/allennlp/claran/catwalk-models/seq-combo-decon-$MODEL_SZ/seq-$(join_by "-" "${SOURCES[@]}")-ALL-upsamp
