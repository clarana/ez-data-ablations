...
export EXPT=${NUM_EPOCH}epoch-decon-paloma-seed-300m-testing
export CHECKPOINT_FOLDER=/data/claran/seed-xppt-${EXPT}
export SAVE_INTERVAL=200ba
...
composer -n $GPUS --stdout stdout_{rank}.log --stderr stderr_{rank}.log --master_port 1234 pretrain-seed.py \
    ...
    --resume_training \
    --eval_perplexity_subsets > >(tee -a /data/claran/test-300m/seed-decon-${NUM_EPOCH}epoch-stdout.log) 2> >(tee -a /data/claran/test-300m/seed-decon-${NUM_EPOCH}epoch-stderr.log >&2)

mkdir /data/claran/test-300m
cp "$CHECKPOINT_FOLDER/$EXPT-metrics.json" /data/claran/test-300m/metrics.json

export checkpt=$(find $CHECKPOINT_FOLDER | grep customgpt-300m-tokens)
echo "$checkpt"
...
#python xp-pt-as-olmo/convert_checkpoint.py --checkpoint $checkpt --config xp-pt-as-olmo/configs/customgpt-300m.yaml --output_dir /net/nfs.cirrascale/allennlp/claran/catwalk-models/seed-300m-decon-${NUM_EPOCH}epoch
#cp /net/nfs.cirrascale/allennlp/claran/catwalk-models/seed-xppt/*.json /net/nfs.cirrascale/allennlp/claran/catwalk-models/seed-300m-decon-${NUM_EPOCH}epoch
