...
export MODEL_SZ=110m

wandb login --relogin $WANDB_KEY

export EXPT_BASE=data_mix_2_upsamp
export SOURCE=m2d2_wiki-toplevel

export EXPT=${EXPT_BASE}-part-$SOURCE-i${IND}-${PART1//[, ]/}-${PART2//[, ]/}

export PARTITION_INFO=/net/nfs.cirrascale/allennlp/claran/m2d2_wiki-top_partition_idx-3-19.json
...
export CHECKPOINT_FOLDER=/results/branch-xppt-${EXPT_BASE}/${SOURCE}/${EXPT_BASE}-part-$SOURCE-i${IND}-${PART1//[, ]/}-${PART2//[, ]/}
...
composer -n $GPUS --stdout stdout_{rank}.log --stderr stderr_{rank}.log --master_port 1234 pretrain-branch.py \
    ...
    --partition_name_list "${PART1}" "${PART2}" \
    --upsample \
...
python xp-pt-as-olmo/convert_checkpoint.py --checkpoint $checkpt --config xp-pt-as-olmo/configs/customgpt-110m.yaml --output_dir /net/nfs.cirrascale/allennlp/claran/catwalk-models/data-mix-upsamp-wiki/branch-train-2-i${IND}-${PART1//[, ]/}-${PART2//[, ]/}
cp /net/nfs.cirrascale/allennlp/claran/catwalk-models/seed-decon-5epoch-${MODEL_SZ}/*.json /net/nfs.cirrascale/allennlp/claran/catwalk-models/data-mix-upsamp-wiki/branch-train-2-i${IND}-${PART1//[, ]/}-${PART2//[, ]/}
