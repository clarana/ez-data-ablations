...
export EXPT_BASE=subinds-seq-fos
export SOURCE=s2orc-recombined
export EXPT=${EXPT_BASE}-part-$SOURCE-${FOS//[, ]/}-i${SUB_IND}-of${NUM_SUBSETS}-${NUM_BIL_TOKS}bil
...
export CHECKPOINT_FOLDER=/results/branch-xppt-${EXPT_BASE}/${SOURCE}/${FOS}-i${SUB_IND}-of${NUM_SUBSETS}-${NUM_BIL_TOKS}bil
...
export EVAL_INTERVAL=$(( (20000 + 500*${NUM_BIL_TOKS}) / 5))ba
#10ba
#1000ba
...
python xp-pt-as-olmo/convert_checkpoint.py --checkpoint $checkpt --config xp-pt-as-olmo/configs/customgpt-110m.yaml --output_dir /net/nfs.cirrascale/allennlp/claran/catwalk-models/subbranch-train-fos-${FOS}-i${SUB_IND}-of${NUM_SUBSETS}-${NUM_BIL_TOKS}bil
cp /net/nfs.cirrascale/allennlp/claran/catwalk-models/seed-xppt/*.json /net/nfs.cirrascale/allennlp/claran/catwalk-models/subbranch-train-fos-${FOS}-i${SUB_IND}-of${NUM_SUBSETS}-${NUM_BIL_TOKS}bil
